        ...
        <div class="page-content">
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <h3 class="page-title">
                        <?php echo $this->lang->line('event_package'); ?>
                    </h3>
                    <ul class="page-breadcrumb breadcrumb">
                        ...
                        <li>
                            <?php echo $this->lang->line('packages') ?>
                        </li>
                    </ul>
                ...
                <div class="col-md-12">
                    ...
                    <div class="portlet box red">
                        <div class="portlet-title">
                            <div class="caption"><?php echo $this->lang->line('packages') ?></div>
                            <div class="actions c-dropdown">
                                <?php if(in_array('restaurant_package~ajaxDisableAll',$this->session->userdata("UserAccessArray"))) { ?>
                                    <button class="btn default-btn btn-sm danger-btn theme-btn" id="active_pkg"><i class="fa fa-check"></i> <?php echo $this->lang->line('active') ?></button>
                                    <button class="btn default-btn btn-sm danger-btn theme-btn" id="deactive_pkg"><i class="fa fa-ban"></i> <?php echo $this->lang->line('inactive') ?></button>
                                <?php } ?>
                                <?php if(in_array('restaurant_package~add_package',$this->session->userdata("UserAccessArray"))) { ?>
                                    ...
                                    <ul class="dropdown-menu pull-right" role="menu">
                                        ...
                                        ?>
                                        <li><a href="<?php echo base_url().ADMIN_URL.'/'.$this->controller_name.'/add_package/'.$lang->language_slug?>"><?php echo $langname; ?></a></li>
                                        <?php } ?>
                                    </ul>
                                <?php } ?>                                
                            </div>
...
<script>
var grid;
var package_count = <?php echo ($package_count)?$package_count:0; ?>;
        ...
        dataTable: {  // here you can define a typical datatable settings from http://datatables.net/usage/options 
           ...
           "aoColumns": [
            ...
            "fnStateSave": function (oSettings, oData) {
                if(oSettings.aoData.length == 0 && package_count != 0 && oData.iStart >= package_count){
                    oData.iStart = 0;
        ...
        callback: function (disableConfirm) {  
            if (disableConfirm) {
                jQuery.ajax({
                  ...
                  dataType : "json",
                  url : 'ajaxDisableAll',
                  data : {'content_id':ContentID,'status':Status,'tblname':'restaurant_package'},
        ...
        callback: function (deleteConfirm) { 
            if (deleteConfirm) {
                jQuery.ajax({
                  ...
                  dataType : "html",
                  url : 'ajaxDelete',
                  data : {'tblname':'restaurant_package','entity_id':entity_id,'content_id':content_id},
                  success: function(response) {
                    grid.getDataTable().fnDraw(); 
                  },
                  error: function(XMLHttpRequest, textStatus, errorThrown) {           
...
function deleteAll(ContentID, message)
{    
    bootbox.confirm({
        message: message,
        buttons: {
        ...
        callback: function (deleteConfirm) {         
          if (deleteConfirm) {
              jQuery.ajax({
                ...
                dataType : "json",
                url : 'ajaxDeleteAll',
                data : {'tblname':'restaurant_package','content_id':ContentID},
                success: function(response) {
                  //grid.getDataTable().fnDraw(); 
                  location.reload();
                },
...
//activate multiple restaurant package
$('#active_pkg').click(function(e){
    e.preventDefault();
            ...
            callback: function (activeConfirm) {  
                if (activeConfirm) {
                    jQuery.ajax({
                      ...
                      data : {'arrayData':CommissionIdComma, 'flag':'active','tab':'pkg'},
                      success: function(response) {
                        grid.getDataTable().fnDraw(); 
                      },
                      error: function(XMLHttpRequest, textStatus, errorThrown) {           
...
//deactivate multiple restaurant package
$('#deactive_pkg').click(function(e){
    e.preventDefault();
    var records = grid.getSelectedRows();  
    if(!jQuery.isEmptyObject(records)){            
        var CommissionIds = Array();
            ...
            callback: function (deactiveConfirm) {  
                if (deactiveConfirm) {
                    jQuery.ajax({
                      type : "POST",
                      url : 'activeDeactiveMultiRes',
                      data : {'arrayData':CommissionIdComma, 'flag':'deactive','tab':'pkg'},
                      success: function(response) {
                        grid.getDataTable().fnDraw(); 
                      },
                      error: function(XMLHttpRequest, textStatus, errorThrown) {           
